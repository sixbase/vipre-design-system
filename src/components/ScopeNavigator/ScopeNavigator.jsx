 ...
 * Composes Surface + Input + Icon. The bar scopes `dark` onto itself, so it (and
 * its popovers) always render on the product navy — light-on-dark — even when the
 * surrounding page is in light mode. It is also container-responsive: the search
 * stacks below the trail when the bar's own width (not the viewport) is narrow.
 *
 * Props:
...
export const ScopeNavigator = forwardRef(function ScopeNavigator(
  ...
  // Stack the search below the trail when the *container* (not the viewport) is
  // narrow — driven off the measured width so it's correct inside any layout,
  // not just at the browser breakpoint.
  const stack = navWidth > 0 && navWidth < 768
  ...
  return (
    <nav
      ...
      className={cx('vds-scope', 'dark', stack && 'vds-scope--stack', className)}
      {...props}
    >
