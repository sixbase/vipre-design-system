...
function ProductCell({ glyph, name, category }) {
  return (
    <Inline gap={2} align="start">
      <ProductTile glyph={glyph} size={24} />
      <Stack gap={0}>
        <Text as="span" variant="body">{name}</Text>
...
function ProductCellCompact({ glyph, name }) {
  return (
    <Inline gap={2}>
      <ProductTile glyph={glyph} size={20} />
...
/>`}
      ...
      <Section
        ...
        note="A product and its name in the first column. The mark is a ProductTile — the product's own square, not a general-purpose icon, which says &quot;something about email&quot; where the tile says &quot;this product&quot;. 24px, because the cell carries a second line. Mark and name live in ONE cell, so there is no mark column to align and no empty header to explain."
      >
        <Preview
          canvas={
            <Table
...
const columns = [
  {
    ...
    render: (r) => (
      ...
      <Inline gap={2} align="start">
        <ProductTile glyph={r.glyph} size={24} />
        <Stack gap={0}>
